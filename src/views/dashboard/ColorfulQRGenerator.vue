    ...
    <el-card class="qr-generator-card">
        <template #header>
            <div class="card-header">
                ...
                <span class="header-title">生成彩色二维码</span>
            </div>
        </template>

        <el-form :model="form" :rules="rules" ref="formRef" :label-width="isMobile ? '80px' : '90px'"
            :label-position="isMobile ? 'top' : 'right'" class="qr-form">
            ...
            <!-- URL 输入 -->
            <el-form-item v-if="form.type === 'url'" label="目标链接" prop="url">
                <el-input v-model="form.url" placeholder="https://example.com" clearable :prefix-icon="Link" type="url"
            ...
            <!-- 文本输入 -->
            <el-form-item v-if="form.type === 'text'" label="文本内容" prop="text">
                <el-input v-model="form.text" type="textarea" :rows="4" placeholder="请输入文本内容" clearable
                    :disabled="loading" />
            </el-form-item>

            <!-- 文件上传 -->
            <el-form-item v-if="['image', 'pdf', 'word'].includes(form.type)" label="上传文件" prop="file">
            ...
            <div class="form-row">
                <el-form-item label="尺寸大小">
                    <el-input-number v-model="form.size" :min="100" :max="1000" :step="50" controls-position="right"
                        :style="{ width: isMobile ? '100%' : '180px' }" />
                    <span class="unit-text">像素</span>
                </el-form-item>

                <el-form-item label="边距">
                    <el-input-number v-model="form.margin" :min="0" :max="80" controls-position="right"
                        :style="{ width: isMobile ? '100%' : '180px' }" />
                    <span class="unit-text">像素</span>
                </el-form-item>
            </div>
            ...
            <div class="form-row">
                <el-form-item label="前景颜色">
                    ...
                    <span class="unit-text hex-text">{{ form.foreground }}</span>
                </el-form-item>

                <el-form-item label="背景颜色">
                    <el-color-picker v-model="form.background" color-format="hex" :predefine="predefineColors" />
                    <span class="unit-text hex-text">{{ form.background }}</span>
                </el-form-item>
            </div>
            ...
            <div class="form-row">
                <el-form-item label="码点样式">
                    <el-select v-model="form.style_dot" placeholder="默认" :style="{ width: isMobile ? '100%' : '180px' }">
                ...
                <el-form-item label="码眼样式">
                    <el-select v-model="form.style_eye" placeholder="默认" :style="{ width: isMobile ? '100%' : '180px' }">
                        <el-option label="方块 (Square)" value="square" />
                        <el-option label="圆角 (Round)" value="round" />
                    </el-select>
                </el-form-item>
            ...
            <div class="form-row">
                <el-form-item label="码眼内框">
                    <el-color-picker v-model="form.eye_color_inner" color-format="hex" :predefine="predefineColors" />
                    <span class="unit-text" v-if="form.eye_color_inner">自定义</span>
                    <span class="unit-text" v-else>跟随前景</span>
                </el-form-item>

                <el-form-item label="码眼外框">
                    <el-color-picker v-model="form.eye_color_outer" color-format="hex" :predefine="predefineColors" />
                    <span class="unit-text" v-if="form.eye_color_outer">自定义</span>
                    <span class="unit-text" v-else>跟随前景</span>
                </el-form-item>
            </div>
            ...
            <div class="form-row">
                <el-form-item label="纠错等级">
                    <el-select v-model="form.ecc" placeholder="纠错等级" :style="{ width: isMobile ? '100%' : '180px' }">
            ...
            <el-form-item class="form-actions">
                <el-button type="primary" :loading="loading" :disabled="isSubmitDisabled" @click="submit"
                    :class="{ 'mobile-btn': isMobile }">
                    <el-icon v-if="!loading" class="btn-icon">
                        <MagicStick />
                    </el-icon>
                    <span>生成彩色二维码并保存</span>
                </el-button>

                <el-button v-if="!isMobile && preview" @click="downloadQR" :disabled="loading">
        ...
        <div v-if="preview" class="preview-section">
            <div class="preview-header">
                ...
                <span class="preview-title">彩色二维码预览</span>
            </div>
            ...
            <div class="preview-content">
                <div class="qr-wrapper">
                    ...
                    <!-- 移动端下载按钮 -->
                    <el-button v-if="isMobile" type="primary" @click="downloadQR" class="mobile-download-btn">
                        <el-icon>
                            <Download />
                        </el-icon>
                        <span>下载二维码</span>
                ...
                <el-alert type="success" :closable="false" show-icon class="success-tip">
                    <template #title>
                ...
                <div class="qr-info">
                    ...
                    <div class="info-item">
                        ...
                        <span class="info-value">{{ form.foreground }}</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">背景色</span>
                        <span class="info-value">{{ form.background }}</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">样式</span>
                        <span class="info-value">点:{{ form.style_dot }} / 眼:{{ form.style_eye }}</span>
                    </div>
                    <div class="info-item">
                        <span class="info-label">纠错</span>
                        <span class="info-value">{{ form.ecc }}</span>
                    </div>
                </div>
            </div>
        </div>
...
import { generateColorfulQRCode, generatePublicColorfulQRCode, checkUrlValidity } from '../../api/qrcode'
...
const isSubmitDisabled = computed(() => {
    ...
    // Check if current type is enabled
    if (!typeSettings.value[form.type]) return true
...
const fetchSettings = async () => {
    if (props.isPublic) return // 公开模式下使用默认配置
    ...
    try {
        ...
        if (res.data) {
            ...
            if (data.colorful_qr_type_url_enabled !== undefined) typeSettings.value.url = toBool(data.colorful_qr_type_url_enabled)
            if (data.colorful_qr_type_text_enabled !== undefined) typeSettings.value.text = toBool(data.colorful_qr_type_text_enabled)
            if (data.colorful_qr_type_image_enabled !== undefined) typeSettings.value.image = toBool(data.colorful_qr_type_image_enabled)
            if (data.colorful_qr_type_pdf_enabled !== undefined) typeSettings.value.pdf = toBool(data.colorful_qr_type_pdf_enabled)
            if (data.colorful_qr_type_word_enabled !== undefined) typeSettings.value.word = toBool(data.colorful_qr_type_word_enabled)
            ...
            if (!typeSettings.value[form.type]) {
                ...
                if (firstAvailable) {
                    ...
                    ElMessage.warning('所有二维码生成功能均已被管理员禁用')
                }
            }
        }
    } catch (e) {
...
const submit = async () => {
    try {
        ...
        const apiFunc = props.isPublic ? generatePublicColorfulQRCode : generateColorfulQRCode
...
const downloadQR = () => {
    ...
    link.download = `colorful_qrcode_${Date.now()}.png`
...
@media (hover: none) and (pointer: coarse) {
    ...
    .qr-form :deep(.el-form-item__content .el-button) {
        -webkit-tap-highlight-color: transparent;
    }
}
